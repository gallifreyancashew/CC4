package expressions;
import java.util.*;
public class PostFixArray { //conversion of infix (user input) to postfix (output)
        public static void main(String[] args) {
        // accept user input for expression to be converted
        String exp;
        Scanner sc = new Scanner(System.in);
            do {
               System.out.println("Enter Infix, put a # after the expression: ");
               exp = sc.nextLine();
    ...
    static int isp (char ch){//in stack priority
        if (ch=='+'||ch=='-'){ 
            return 1; 
        } else if (ch=='*'||ch=='/'||ch=='%'){ 
            return 2;  
    ...
    static String toPostfix (String exp){
        ...
        for (int i=0; i<exp.length();i++){
            ...
            if (i!=exp.length()-1)
                 System.out.printf("%-15.30s %-15.30s %-15.30s%n",token,"#"+stack.toString().replaceAll("\\[","").replaceAll("]",""),postfix);
            else
                System.out.printf("%-15.30s %-15.30s %-15.30s%n",token,stack.toString().replaceAll("\\[","").replaceAll("]",""),postfix);
        }
        return postfix;
